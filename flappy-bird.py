...
def main():
	...
	while run:
		...
		for pipe in pipes:
			# collision detection 
			if pipe.collide(birdObject):
				pass

			if pipe.x + pipe.PIPE_TOP.get_width() < 0: # if pipe is completely off the screen, remove that pipe
				rem.append(pipe)

			if not pipe.passed and pipe.x < birdObject.x: # as soon as the pipe is passed generate a new one
				pipe.passed = True
				add_pipe =True
		...
		if birdObject.y + birdObject.img.get_height() >= 730: # we hit the floor
			pass

		base.move()
		draw_window(window, birdObject, pipes, base, score)
